...
DrawingTransform = class DrawingTransform {
  ...
  init() {
    var bbox, center, matrix, padding, referentiel, time, transform_handle_center, transform_handle_center_distance;
    if (this.last_init) {
    ...
    this.position_handle = new DrawingHandle(this.svg, center, {
      ...
      move: (position) => {
        ...
        this.transform_handle.destroy();
        translation_matrix = [[1, 0, position[0]], [0, 1, position[1]], [0, 0, 1]];
        return DrawingUtils.apply_matrix(this.element, MatrixUtils.mult(translation_matrix, matrix));
      },
      end: (position) => {
        this.options.end();
        return this.init();
    ...
    transform_handle_center = MatrixUtils.multVector(matrix, [bbox.x + bbox.width + padding, bbox.y + bbox.height + padding, 1]);
    transform_handle_center_distance = Geometry.distance(center, [transform_handle_center[0], transform_handle_center[1]]);
    return this.transform_handle = new DrawingHandle(this.svg, transform_handle_center, {
      color: '#0070d2',
      element: () => {
        var group, icon, scale, size, translate;
        group = DrawingUtils.create_element(this.svg, 'g');
        size = Geometry.distance(referentiel.global_to_local([0, 0]), referentiel.global_to_local([20, 20]));
        ...
        DrawingUtils.apply_matrix(group, [[1, 0, transform_handle_center[0]], [0, 1, transform_handle_center[1]], [0, 0, 1]]);
        return group;
      },
      move: (move) => {
        var angle, position, scale, transformation_matrix;
        this.position_handle.destroy();
        position = [transform_handle_center[0] + move[0], transform_handle_center[1] + move[1]];
        angle = Geometry.angle(center, transform_handle_center, position);
        scale = 1 + (Geometry.distance(center, position) - transform_handle_center_distance) / transform_handle_center_distance;
        transformation_matrix = Geometry.multiply_matrix([[1, 0, center[0]], [0, 1, center[1]], [0, 0, 1]], [[scale, 0, 0], [0, scale, 0], [0, 0, 1]], Geometry.rotation_matrix(angle), [[1, 0, -center[0]], [0, 1, -center[1]], [0, 0, 1]]);
        return DrawingUtils.apply_matrix(this.element, Geometry.multiply_matrix(transformation_matrix, matrix));
      },
      cancel: (position) => {
        return this.init();
      },
