...
class MoveListener {
  constructor (element, options) {
    ...
    window.requestAnimationFrame(this.loop_callback)
  }
  ...
  tick () {
    ...
    if (this._touches != null) {
      ...
      for (j = 0, len = ref.length; j < len; j++) {
        touch = ref[j]
        touches.push(this._referentiel.globalToLocal(touch))
      }
    }
    if (touches.length > 0) {
      this.options.move(touches)
    }
    return window.requestAnimationFrame(this.loop_callback)
  }
